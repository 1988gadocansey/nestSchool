...
function checkempty($item,$mainvalue){
	if($mainvalue=='' or $mainvalue==0){return "-";}else{return $item;}
	}

function picturesp($path,$target){
	if(file_exists($path)){
...
function sendtxt($message,$phone,$type,$name) 
{ 
set_time_limit(500);
...
$url="http://powertxtgh.com/access.php?company=ALOT&ccode=ALT101&sender=HOLYCHILD&message=$themassage&recipient=$phone";

$f=@fopen($url,"r"); 

$date=time();
	 $insertor="insert into sent set number='$phone',type='$type',name='$name',message='$message',dates='$date',status='Delivered'";
	mysql_query($insertor) or die(mysql_error());
...
return true; 
	 ...
	 $insertor="insert into sent set number='$phone',type='$type',name='$name',message='$message',dates='$date',status='Not Delivered'";
	mysql_query($insertor) or die(mysql_error());
...
function ping($host, $port, $timeout) { 
  ...
  function buildall($data,$part){
	if($data!='ALL,' and $data){
	//echo $data;echo "<br/>";
	  if($part!='notid'){	
	$new=" and ( 1='' "; }else{			
	$new=" and ( 1!='' "; 
  ...
  foreach($all as $dd){
	  ...
	  if($part!='notid' and $dd and $dd!='ALL'){
	  $new.=" or $part='$dd'";
	  }elseif($part=='notid' and $dd){
