...
return $currentTerm+1;
 ...
 public function getIndexNo1($program){
     ...
     return $data->name.'-'.date("y");
    
    }
 public function getName($stuId){
     $data= \DB::table('student')->where("indexNo",$stuId)->first();
     return $data->name;
 ...
 public function getCourseGrade($courseId,$year,$term,$student) {
     ...
     $data= \DB::table('assesmentsheet')->where("indexNo",$student)
             ->where("year",$year)
             ->where("term",$term)
 ...
 public function getShortCode($course) {
    ...
    public function getTotalPassed($indexno) {
         
       $array=  $this->getSemYear();
            $sem=$array[0]->term;
          $year=$array[0]->year;
         $total = \DB::table('assesmentsheet')->where("indexno",$indexno)
                 ->where("term", $sem)->where("year",$year)->where("total",">=","50.00")
                ->count();
         return $total;
       
         
    }
    public function getTotalFailded($indexno) {
     ...
     public function getTotalUnComplete($indexno) {
         
       $array=  $this->getSemYear();
            $sem=$array[0]->term;
          $year=$array[0]->year;
         $total = \DB::table('assesmentsheet')->where("indexno",$indexno)
                 ->where("term", $sem)->where("year",$year)->where("total","0")
                ->count();
         return $total;
       
         
    }
     public function getTotalSubject($indexno) {
         
       $array=  $this->getSemYear();
            $sem=$array[0]->term;
          $year=$array[0]->year;
         $total = \DB::table('assesmentsheet')->where("indexno",$indexno)
                 ->where("term", $sem)->where("year",$year)->where("courseCode","!=","")
                ->count();
         return $total;
       
         
    }
       public function getClassSize($class) {
         
         $array=  $this->getSemYear();
            $sem=$array[0]->term;
          $year=$array[0]->year;
         $total = \DB::table('classmembers')->where("class",$class)
                 ->where("term", $sem)->where("year",$year)
                ->count();
         return $total;
...
//        }
         ...
         $staff=@\Auth::user()->fund;
         if( @\Auth::user()->department=='top' ||@\Auth::user()->department=='Finance'||@\Auth::user()->role=='Admin'){
           $class = \DB::table('classes')->groupBy('name')->orderby("name")
                ->lists('name', 'name');
         ...
         $class = \DB::table('classes')->where("teacherId",$staff)->groupBy('name')->orderby("name")
                ->lists('name', 'name');
...
if(@\Auth::user()->role=='Lecturer' || @\Auth::user()->role=='HOD' ||@\Auth::user()->role=='Dean'){
    ...
    public function getMountedCourseList() {
         ...
         $query= Models\CourseModel::where("code","!=","")->list("code","name");
         return $query;
             
         
    }
    public function getMountedCourseListLecturer($staffID) {
 $array=  $this->getSemYear();
     $term=$array[0]->term;
   $year=$array[0]->year;
         $query= \DB::table('subjectallocations')->where("subject","!="," ")
                 ->where("year",$year)
                 ->where("term",$term)
                 ->where("teacherId",$staffID)->lists("subject","subject");
         return $query;
...
//    
   public function firesms($message,$phone,$receipient){
        ...
        if (!empty($phone)&& !empty($message)&& !empty($receipient)) {
            ...
            $key = "bcb86ecbc1a058663a07gad"; //your unique API key;
            $message=urlencode($message); //encode url;
        $sender_id="NEST";
 $url = "https://apps.mnotify.net/smsapi?key=$key&to=$phone&msg=$message&sender_id=$sender_idgad";
          //print_r($url);
    ...
    public function getTotalStudentOwing($program,$level){
     ...
     public function getStaffAccount($id){
        ...
        $staff = \DB::table('staffs')->where('emp_number',$id)->get();
    ...
    public function getCourseProgrammeMounted($course) {
       ...
       $progInfo= \DB::table('courses')->where('code',$course)->get();
    ...
    public function getGrade($mark,$type){
     ...
     public function getGradeScore($mark,$type){
        if($mark!=NULL){
        $grade = \DB::table('gradingsystem')
                ->where('lower','<=',$mark)
                ->where('upper','>=',$mark)
    ...
    public function getCourseCodeByIDArray($id){
         ...
         $course= \DB::table('courses')->where('code',$id)
              ->get();
    ...
    public function getCourseObject($code){
     ...
     public function getTotalFeeByProrammeLevel(Request $request){
        
        $data = Models\StudentModel::query();
               if ($request->has('sex') && trim($request->input('sex')) != "") {
            $data->where("gender", "=", $request->input("sex", ""));
        }
         if ($request->has('forms') && trim($request->input('forms')) != "") {
            $data->where("currentClass", "LIKE", "%".$request->input("forms", "")."%");
        }
         if ($request->has('stuType') && trim($request->input('stuType')) != "") {
            $data->where("stuType", $request->input("stuType", ""));
        }
        
        return @count($data->where("status","In School")->get());
