...
class FeeController extends Controller {
    ...
    public function billType() {
         ...
         $query = \DB::table('billmanager')->orderBy("type")
                ->lists('type', 'type');
         return $query;
    }
    ...
    public function getIndex(Request $request) {
        ...
        $sem = $array[0]->term;
        $year = $array[0]->year;
        $fee = Models\BillModel::query();

         if ($request->has('forms') && trim($request->input('forms'))!= "") {
           $fee->where("classes", "LIKE", "%".$request->input('forms')."%");
        }
        ...
        if ($request->has('classes') && trim($request->input('classes'))!= "") {
            $fee->where("classes", "=", $request->input('classes'));
        }
        if ($request->has('term') && trim($request->input('term'))!= "") {
            $fee->where("term", "=", $request->input('term'));
        }
        if ($request->has('type') && trim($request->input('type'))!= "") {
            $fee->where("type", "=", $request->input('type'));
        }
        if ($request->has('gender') && trim($request->input('gender'))!= "") {
            $fee->where("gender", "=", $request->input('gender'));
        }


        $data = $fee->orderBy('classes', 'ASC')->paginate(100);
        ...
        foreach ($data as $key => $row) {
            $total[] = $row->amount;
            $data[$key]->TOTALS = array_sum($total);
            $data[$key]->TOTALSTUDENTS = @$sys->getTotalFeeByProrammeLevel($request);
            $data[$key]->TOTALAMOUNT = @$sys->getTotalFeeByProrammeLevel($request) * $row->amount;
    ...
    public function showBillSignleForm() {
        return view('finance.fees.singleBill');
    }

    
    public function printSingleBill(Request $request, SystemController $sys) {
        ...
        $sql = StudentModel::where("indexNo", $student)->orWhere("cardNo", $student)->orWhere("name","LIKE","%". $student."%")->first();

         
        $academicBill = Models\BillModel::where("classes", $sql->currentClass)->where("year",$year)->where("term",$sem)->where("stuType",$sql->studentType)->where("type","Academic")->get();
       //dd($academicBill);
        $ptaBill = Models\BillModel::where("classes", $sql->currentClass)->where("year",$year)->where("term",$sem)->where("stuType",$sql->studentType)->where("type","PTA")->get();
        $others = Models\BillModel::where("classes", $sql->currentClass)->where("year",$year)->where("term",$sem)->where("stuType",$sql->studentType)->where("type","Others")->get();
        
        if (count($sql) == 0) {
            echo "<script>alert('Student does not exist')</script>";

            // return view('finance.fees.singleBill')->with("error", "<span style='font-weight:bold;font-size:13px;'> $request->input('q') does not exist!</span>");
        } else {
            
            return view("finance.fees.printSingleBill")->with('data', $sql)->with('year', $year)->with('sem', $sem )
                    ->with("others", $others)->with("academic", $academicBill)->with("pta",$ptaBill)
                    ;
        }
    }
    ...
    public function showStudent(Request $request, SystemController $sys) {
        ...
        $sql = StudentModel::where("indexNo", $student)->orWhere("cardNo", $student)->orWhere("name","LIKE","%". $student."%")->get();

        if (@\Auth::user()->department != 'Finance') {
            $sql = StudentModel::where("indexNo", $student)->orwhere("cardNo", $student)->whereHas('programme', function($q) {
                        $q->whereHas('departments', function($q) {
                            $q->whereIn('DEPTCODE', array(@\Auth::user()->department));
                        });
                    })->get();
        }
        $paymentHistory = Models\FeePaymentModel::where("stuId", $student)->take(20)->paginate();
        if (count($sql) == 0) {
            ...
            $sem = $array[0]->year;
            $year = $array[0]->term;
            return view("finance.fees.processPayment")->with('data', $sql)->with('year', $year)->with('sem', $sem)->with('banks', $this->banks())
                            ->with('receipt', $this->getReceipt())->with("class", $sys->getClassList())
                            ->with("payment", $paymentHistory);
        }
    }
...
$academicLeft=0.00;$ptaLeft=0.00; $boardingLeft=0.00;  $totalLeft=0.00;
 //$academicLeft = $academicoutstanding - $amount;
                      ...
                      //dd("hy");
                       ...
                       // dd($smsOwing);
                        ...
                        $smsOwe = $smsOwing[0]->academicOwing + $smsOwing[0]->BoardingOwing + $smsOwing[0]->ptaOwing;
    ...
    public function printOldReceipt(Request $request) {
        ...
        if (@\Auth::user()->department == "Finance") {
            if ($request->isMethod("get")) {
                ...
                $sql = StudentModel::where("indexNo", $student)->orwhere("cardNo", $student)->get();
               // dd($sql);
                if (count($sql) == 0) {
                    ...
                    $receiptQuery = FeePaymentModel::where("stuId", $indexNo)->where("year", $year)->where("term", $sem)->first();
...
//echo $thea[0];
 ...
 public function updateBills(Request $request, SystemController $sys) {
            ...
            $applied= $request->input('valid');
             ...
             dd($request);
                    for ($i = 0; $i < count($id); $i++) {
                        ...
                        Models\FeeModel::where("id",$ids)->update(array(
                 ...
                 return redirect()->back()->with("success", " <span style='font-weight:bold;font-size:13px;'> Data successfully updated</span> ");
