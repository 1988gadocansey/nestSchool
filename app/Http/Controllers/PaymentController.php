...
class PaymentController extends Controller
{
    ...
    public function pad_receiptno($receiptno){
       return str_pad($receiptno, 12, "0", STR_PAD_LEFT);
        ...
        if ($request->has('classes') && trim($request->input('class')) != "") {
        ...
        if ($request->has('users') && trim($request->input('users'))) {
            $fee->where("work", "=", $request->input('users'));
        }
        ...
        if ($request->has('from_date') && $request->has('to_date')) {
            ...
            $fee->whereBetween(\DB::raw('date'), array($request->input('from_date'), $request->input('to_date')));
        }
        if ($request->has('filter') && trim($request->input('filter')) != "" && $request->input('amount') != "") {
            $filter = $request->input('filter');
            $amount = $request->input('amount');
            if ($filter == '=') {
                $fee->where("paid", $amount);
            } else {
                $fee->where("paid", "$filter", $amount);
                // dd($request);
            }
        }

        $data = $fee->orderBy('dates', 'DESC')->paginate(800000);
